...
class TeymorBoardView (context: Context, val n: Int): View(context){
    private  val paint=Paint()
    private  var contairenwidth:Int=0
    private  var size =0
    private  val mat =Array(n){Array(n){
        TeymorBlock(context,0,0F,0F,0F)
    } }
    ...
    fun initGame(){
        ...
        size=contairenwidth/n
        var x=0
        var y=0
        var ID=1
        for (i in 0 until mat.size){
            for (j in 0 until mat[0].size){
                mat[i][j]= TeymorBlock(context,ID,x.toFloat(),y.toFloat(),size.toFloat())
                ID++
                ID %=n*n
                x+=size
            }
