...
class TeymorBlock  (private  val context: Context,
                    var ID:Int, private val x: Float, private  val y: Float,
                    private val size:Float){
    private val textSize=20
    private val strokWidth=3
    ...
    fun onDraw(canvas: Canvas,paint: Paint){
        ...
        paint.color=ContextCompat.getColor(context,R.color.gameBackground)
        paint.strokeWidth=strokWidth*context.resources.displayMetrics.scaledDensity
        canvas.drawRect(x,y,x+size,y+size,paint)
    }
}